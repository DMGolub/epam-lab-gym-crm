...
import static com.epam.dmgolub.gym.controller.mvc.constant.Constants.LOGIN_INDEX_VIEW_NAME;
...
class LoginControllerTest {
	...
	class LogInTest {
		...
		void logIn_shouldRedirectToProfileIndex_whenUserAuthenticatedSuccessfully() {
			...
			when(mapper.mapToCredentials(requestDTO)).thenReturn(request);
			...
			verify(bindingResult, times(1)).hasErrors();
			verify(mapper, times(1)).mapToCredentials(requestDTO);
